...
//!
//! On first run (or when the saved device can no longer be found), Continuum asks
...
//!
//! The choice is saved to `~/.continuum-dev/config.toml` under
//! `[audio].device_name` + `[audio].device_index`. On subsequent runs Continuum
