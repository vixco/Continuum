...
//!
//! The continuum-mcp server runs as a subprocess of the claude CLI, which runs as a
//! subprocess of the main continuum runtime. There is no tokio channel reaching
...
/// Subdirectory under the Continuum data dir where MCP writes spawn/cancel intents.
pub const INTENTS_SUBDIR: &str = "worker-intents";
/// Subdirectory under the Continuum data dir where the runtime publishes snapshots.
...
/// Returns the intents directory for a given Continuum data dir, creating it on
/// first call.
...
/// Returns the snapshots directory for a given Continuum data dir, creating it on
/// first call.
