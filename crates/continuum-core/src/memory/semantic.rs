...
//! Semantic memory represents things Continuum "just knows" — stable knowledge
...
/// The source of a semantic fact — how Continuum learned it.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum FactSource {
    /// The user explicitly stated this fact.
    UserStated,
    /// Continuum observed this from perception frames.
    Observed,
    /// Continuum inferred this from patterns in episodic memory.
    Inferred,
}
...
/// A single semantic fact stored in Continuum's knowledge base.
///
/// Facts are keyed by a dotted path like `user.name` or `project.continuum.stack`.
...
mod tests {
    ...
    async fn test_query_by_prefix() {
        ...
        let continuum_facts = store
            .query_facts_by_prefix("project.continuum")
            .await
            .unwrap();
        assert_eq!(continuum_facts.len(), 2);
    ...
    async fn test_edge_operations() {
        ...
        let edge = Edge {
            ...
            to_key: "project.continuum".to_string(),
        ...
        let edges_to = store.edges_to("project.continuum").await.unwrap();
        assert_eq!(edges_to.len(), 1);
        assert_eq!(edges_to[0].from_key, "user.toshan");

        // Remove one edge.
        let removed = store
            .remove_edge("user.toshan", "project.continuum", "works_on")
            .await
            .unwrap();
        assert!(removed);
