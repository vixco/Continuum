...
//!
//! Optional env:
//! - `CONTINUUM_BENCH_N` — number of iterations (default 10)
//! - `CONTINUUM_BENCH_LANG` — language to benchmark: `en` (default) or `nl`
...
fn main() -> Result<()> {
    init_tracing();
    println!("Continuum voice latency benchmark");
    println!("──────────────────────────────");

    let n: usize = std::env::var("CONTINUUM_BENCH_N")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(10);
    let lang = std::env::var("CONTINUUM_BENCH_LANG").unwrap_or_else(|_| "en".to_string());
