...
/// Mandatory front door for Agent OS. The inner server still owns policy,
/// computer use, Composio and evidence. This wrapper adds cross-process
/// serialization, write-ahead dispatch records, replay prevention and typed
/// postconditions before a plan is allowed to advance.
#[derive(Clone)]
...
impl ReliableAgentOsServer {
    pub fn new(inner: AgentOsServer, data_dir: &Path) -> Result<Self> {
        ...
        std::fs::create_dir_all(&root).with_context(|| {
            format!(
                "failed to create reliable Agent OS run directory {}",
                root.display()
            )
        })?;
    ...
    async fn execute_plan(
        ...
        let plan: RunPlanRequest = serde_json::from_value(arguments).map_err(|error| {
        ...
        let mutating = classifications
            .iter()
            .any(|classification| classification.risk.is_mutating());

        if mutating && !plan.verify_each_step {
            return Ok(visible_error(
                "Mutating Agent OS plans must keep verify_each_step=true.",
            ));
        }
        if mutating
            && plan
        ...
        {
            return Ok(visible_error(
                "Mutating Agent OS plans require a stable run_id. Reuse the exact run_id after reconnects so Continuum can prevent duplicate side effects.",
            ));
        }
            ...
            let steps = classifications
                ...
                .map(|classification| {
                    serde_json::json!({
                        ...
                        "verification_contract": render_contract(&classification.contract),
                        "continue_on_error": classification.continue_on_error
                    })
                })
                .collect::<Vec<_>>();
        ...
        if let Some(answer) = self
            .terminal_or_blocked_result(&run_id, &plan_hash)
            .map_err(internal_error)?
        {
            return Ok(answer);
        }

        // A create-new lock is visible to every Agent OS process using this
        // data directory. We intentionally do not auto-delete stale locks:
        // after a process crash, fail-closed is safer than replaying a mutation.
        let _run_lock = match acquire_run_lock(&self.root, &run_id) {
            Ok(lock) => lock,
            Err(error) => return Ok(visible_error(&error.to_string())),
        };

        // Re-read after taking the cross-process lock because another process
        // may have completed the run between the first read and lock creation.
        if let Some(answer) = self
            .terminal_or_blocked_result(&run_id, &plan_hash)
            .map_err(internal_error)?
        {
        ...
        let mut journal = self
            ...
            .unwrap_or_else(|| ReliableJournal {
                ...
                steps: classifications
                    .iter()
                    .map(|classification| JournalStep {
                        index: classification.index,
        ...
        for classification in &classifications {
            ...
            match journal.steps[index].state {
                StepState::Verified => {
                ...
                StepState::Dispatched | StepState::Unknown => {
                    journal.state = JournalState::Unknown;
                    journal.updated_at = Utc::now();
                    self.save_journal(&journal).map_err(internal_error)?;
                    return Ok(visible_error(
                        "This run contains a mutation that was dispatched without a durable verified outcome. Automatic replay is blocked; inspect the destination before creating a corrective plan.",
                    ));
                }
                _ => {}
            }

            journal.steps[index].state = if classification.risk.is_mutating() {
                // Persist this before entering the inner tool handler. A crash
                // at any point after this line turns the next resume into a
                // manual reconciliation rather than a duplicate side effect.
            ...
            journal.steps[index].verification = None;
            journal.steps[index].result_summary = Value::Null;
            journal.steps[index].error = None;
            journal.updated_at = Utc::now();
            self.save_journal(&journal).map_err(internal_error)?;

            let arguments = step.arguments.as_object().cloned().ok_or_else(|| {
                McpError::invalid_params(
                    format!("step {} arguments must be a JSON object", index + 1),
                    None,
                )
            })?;
            ...
            let result = match call_result {
                ...
                Ok(result) => {
                    ...
                    if classification.risk.is_mutating() && !is_known_pre_dispatch_failure(&message)
                    {
                        mark_unknown(&mut journal, index, &message);
                        self.save_journal(&journal).map_err(internal_error)?;
                        return Ok(visible_error(&format!(
                            "Step {} returned an ambiguous mutation error. Its outcome is unknown and it will not be replayed automatically. {message}",
                            index + 1
                        )));
                    }
                    had_errors = true;
                    mark_failed(&mut journal, index, &message);
                    self.save_journal(&journal).map_err(internal_error)?;
                    if classification.continue_on_error {
                ...
                Err(error) => {
                    ...
                    if classification.risk.is_mutating() && !is_known_pre_dispatch_failure(&message)
                    {
                        ...
                        self.save_journal(&journal).map_err(internal_error)?;
                        return Ok(visible_error(&format!(
                            "Step {} lost a reliable response after dispatch. Its external outcome is unknown and automatic replay is blocked. {message}",
                            index + 1
                        )));
                    }
                    had_errors = true;
            ...
            if verification.status != "verified" {
                ...
                if classification.risk.is_mutating() {
                ...
                had_errors = true;
                mark_failed(&mut journal, index, &detail);
                self.save_journal(&journal).map_err(internal_error)?;
                if classification.continue_on_error {
                    response_steps.push(failed_step_json(&journal.steps[index]));
                    continue;
    ...
    fn terminal_or_blocked_result(
        &self,
        run_id: &str,
        plan_hash: &str,
    ) -> Result<Option<CallToolResult>> {
        let Some(journal) = self.load_journal(run_id)? else {
            return Ok(None);
        };
        if journal.plan_hash != plan_hash {
            return Ok(Some(visible_error(
        ...
        match journal.state {
            JournalState::Completed | JournalState::CompletedWithErrors => Ok(Some(json_result(
                &journal.final_result.unwrap_or_else(|| {
                    serde_json::json!({
                        "reliable": true,
                        "run_id": run_id,
            ...
            JournalState::Unknown => Ok(Some(visible_error(
                "This run has an unknown external outcome. Continuum will not replay it automatically. Inspect the destination and journal, then create a new corrective plan.",
            ))),
            _ if journal.steps.iter().any(|step| {
                matches!(step.state, StepState::Dispatched | StepState::Unknown)
            }) => Ok(Some(visible_error(
    ...
    async fn verify_postcondition(
        ...
        match condition {
            ...
            Postcondition::JsonPointerExists(pointer) => {
                let exists = result.pointer(pointer).is_some();
                VerificationRecord {
                    status: if exists { "verified" } else { "contradicted" }.to_string(),
            ...
            Postcondition::JsonPointerEquals { pointer, expected } => {
                ...
                VerificationRecord {
                    status: if matches { "verified" } else { "contradicted" }.to_string(),
            ...
            Postcondition::WindowTitleContains(needle) => {
                ...
                .unwrap_or_default();
                let observe =
                    CallToolRequestParams::new("computer_observe").with_arguments(arguments);
                match self.inner.call_tool(observe, context).await {
                    ...
                    Ok(value) => VerificationRecord {
                        ...
                        contract: "window_title_contains".to_string(),
                        detail: format!(
                            "Post-action window observation failed: {}",
                            result_text(&value)
                        ),
                    },
            ...
            Postcondition::ElementPresent(selector) => {
                ...
                match self.inner.call_tool(find, context).await {
                    Ok(value) if value.is_error != Some(true) => VerificationRecord {
                        ...
                        detail: "The semantic UI element was present, visible and enabled."
                            .to_string(),
                    },
                    Ok(value) => VerificationRecord {
    ...
    fn save_journal(&self, journal: &ReliableJournal) -> Result<()> {
        ...
        if path.exists() {
            std::fs::rename(&path, &backup).with_context(|| {
                format!(
                    "failed to preserve current reliable journal {}",
                    path.display()
                )
            })?;
        }
        if let Err(error) = std::fs::rename(&temporary, &path) {
            ...
            return Err(error).with_context(|| {
                format!("failed to activate reliable journal {}", path.display())
            });
        }
        if backup.exists() {
...
fn validate_plan(plan: &RunPlanRequest) -> Result<Vec<StepClassification>> {
    ...
    for (index, step) in plan.steps.iter().enumerate() {
        ...
        if risk.is_mutating() && step.continue_on_error {
            bail!(
                "mutating step {id:?} cannot continue_on_error; uncertainty must halt later actions"
            );
        }
        let contract = parse_postcondition(step.expectation.as_deref(), risk)?;
        output.push(StepClassification {
            index,
            id,
...
fn parse_postcondition(value: Option<&str>, risk: ReliableRisk) -> Result<Postcondition> {
    ...
    if value == "result_ok" {
        if risk.is_mutating() {
            bail!("result_ok is insufficient for a mutating step; verify an observable outcome");
        }
        return Ok(Postcondition::ResultOk);
    }
    ...
    if let Some(rest) = value.strip_prefix("json_pointer_equals:") {
        let (pointer, expected) = rest
            .split_once('=')
            .ok_or_else(|| anyhow::anyhow!("json_pointer_equals must use /pointer=<json>"))?;
        validate_pointer(pointer)?;
        let expected = serde_json::from_str(expected)
            .context("json_pointer_equals expected value is not valid JSON")?;
...
fn validate_needle(value: &str, max_chars: usize, name: &str) -> Result<()> {
    if value.trim().is_empty() || value.chars().count() > max_chars {
        bail!("{name} requires between 1 and {max_chars} characters");
    }
    Ok(())
}
...
fn classify_step(step: &PlanStep) -> ReliableRisk {
...
fn hash_plan(plan: &RunPlanRequest) -> String {
...
fn acquire_run_lock(root: &Path, run_id: &str) -> Result<RunFileLock> {
    ...
    let mut file = match options.open(&path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
            bail!(
                "run_id {run_id:?} is already executing or requires manual reconciliation after a crashed process"
            )
        }
...
fn is_known_pre_dispatch_failure(message: &str) -> bool {
...
fn mark_failed(journal: &mut ReliableJournal, index: usize, message: &str) {
...
fn object_key_contains(value: &Value, key: &str, needle: &str) -> bool {
    ...
    object_key_contains_lower(value, key, &needle)
}

fn object_key_contains_lower(value: &Value, key: &str, needle: &str) -> bool {
    match value {
        ...
        Value::Array(items) => items
            .iter()
            .any(|value| object_key_contains_lower(value, key, needle)),
        _ => false,
    }
}
...
mod tests {
    ...
    fn step(action: &str, expectation: Option<&str>) -> PlanStep {
        PlanStep {
            id: None,
    ...
    #[test]
    fn mutating_continue_on_error_is_rejected() {
        let plan = RunPlanRequest {
    ...
    fn plan_hash_is_stable_across_object_key_order() {
        ...
        let right = RunPlanRequest {
            steps: vec![right],
            ..left.clone()
        };
        assert_eq!(hash_plan(&left), hash_plan(&right));
    }
    ...
    #[test]
    fn mutating_direct_tools_are_closed() {
        assert!(is_direct_mutation("computer_click"));
        assert!(is_direct_mutation("composio_create_session"));
        assert!(!is_direct_mutation("computer_observe"));
    }
}
