...
//! any real Continuum tools. If this example runs and claude can invoke its `echo` tool
//! via --mcp-config, the protocol wiring is sound.
//!
//! Run manually:
//!   cargo build --example echo_smoke -p continuum-mcp
...
impl ServerHandler for EchoServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions("Continuum Phase 4 echo smoke-test server.")
    }
}
...
async fn main() -> Result<()> {
    ...
    tracing::info!("continuum-echo-smoke: starting stdio server");
