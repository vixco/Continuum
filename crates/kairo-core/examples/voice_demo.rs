...
//! the wake phrase, e.g. `hey kairo what's the weather` — the demo
...
fn main() -> Result<()> {
    ...
    println!("Kairo voice demo — Phase 5C acceptance gate");
    println!("────────────────────────────────────────────");
    ...
    loop {
        ...
        let utterance = if followup_active {
            ...
            println!("  ✗ No wake phrase, not addressed to Kairo.");
            continue;
        };
        ...
        // Wait until Kairo finishes talking before prompting again.
        speech.wait_idle();
