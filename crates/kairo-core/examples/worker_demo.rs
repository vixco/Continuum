...
//! spawned — set `KAIRO_WORKER_DRY_RUN=0` to exercise the real subprocess.
//!
//! ```bash
//! cargo run --release --example worker_demo -p kairo-core
//! ```
...
use anyhow::Result;
use kairo_core::skills::SkillLoader;
use kairo_core::workers::{WorkerPool, WorkerPoolOptions, WorkerSpec};
...
async fn main() -> Result<()> {
    ...
    // Anthropic API. Override with `KAIRO_WORKER_DRY_RUN=0` for a live run.
    if std::env::var_os("KAIRO_WORKER_DRY_RUN").is_none() {
