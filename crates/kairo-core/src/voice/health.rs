...
mod tests {
    ...
    fn nonexistent() -> std::path::PathBuf {
        std::path::PathBuf::from("/definitely/does/not/exist/kairo-health-test")
    }
    ...
    fn wake_disabled_status() {
        let h = wake_health(false, "hey kairo");
        assert_eq!(h.status, HealthStatus::Disabled);
    }

    #[test]
    fn wake_healthy_normal_keyword() {
        let h = wake_health(true, "hey kairo");
        assert_eq!(h.status, HealthStatus::Healthy);
        assert!(h.detail.contains("hey kairo"));
    }
