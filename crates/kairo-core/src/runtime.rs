//! # Embeddable Kairo runtime
//!
//! The desktop app needs the same four-layer pipeline the `kairo` binary
//! runs, but with hooks for the dashboard: a shared state store, a log
//! buffer, and a way to pause / resume and trigger repair.
//!
//! [`KairoRuntime`] owns the [`StateHandle`], [`LogBuffer`],
...
use crate::config::{kairo_dev_dir, load_config, KairoConfig};
...
#[derive(Clone)]
pub struct KairoRuntime {
    pub state: StateHandle,
    pub logs: LogBuffer,
    pub automations: AutomationStore,
    inner: Arc<RwLock<KairoRuntimeInner>>,
    shutdown_tx: watch::Sender<bool>,
}

struct KairoRuntimeInner {
    config: KairoConfig,
    config_path: PathBuf,
    dev_dir: PathBuf,
    paused: bool,
...
impl KairoRuntime {
    ...
    /// `~/.kairo-dev/config.toml` (created on demand).
    pub fn init() -> Result<Self> {
        ...
        Ok(Self {
            ...
            automations,
            inner: Arc::new(RwLock::new(KairoRuntimeInner {
                config,
                config_path,
                dev_dir,
                paused: false,
    ...
    pub fn config_snapshot(&self) -> KairoConfig {
    ...
    /// push).
    pub fn update_config<F>(&self, updater: F) -> Result<KairoConfig>
    where
        F: FnOnce(&mut KairoConfig),
    {
        let (snapshot, path) = {
...
fn persist_config(path: &Path, cfg: &KairoConfig) -> Result<()> {
