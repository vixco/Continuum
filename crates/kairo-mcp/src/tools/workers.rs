//! # Worker tools (`mcp__kairo__workers_*`)
//!
...
//! intent that the running `kairo` runtime picks up on its next tick.
//!
...
use chrono::Utc;
use kairo_core::workers::intent::{list_snapshots, read_snapshot, write_intent, WorkerIntent};
use kairo_core::workers::types::{
...
mod tests {
    ...
    fn list_returns_only_matching_status() {
        ...
        kairo_core::workers::intent::write_snapshot(tmp.path(), &a).unwrap();
        kairo_core::workers::intent::write_snapshot(tmp.path(), &b).unwrap();
    ...
    async fn wait_returns_completed_when_snapshot_exists() {
        ...
        kairo_core::workers::intent::write_snapshot(tmp.path(), &snap).unwrap();
