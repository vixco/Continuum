...
If the draft teaches Kairo something durable (a new recipient, a
preferred tone shift, a recurring meeting mention), write it to
semantic memory at the end:

```
mcp__kairo__memory_set_fact(key="contact.<name>.email", value=...)
mcp__kairo__memory_set_fact(key="contact.<name>.email_tone", value=...)
```
