...
- Memory tools (`mcp__continuum__memory_*`) hold what Continuum already knows. Check them **first** before asking the user a fact that might already be stored. Semantic facts live at dotted keys like `user.name` or `project.simcharts.dir`; episodic events are past wakes, responses, and prior tool calls.
