//! Provider-neutral memory tools for the desktop chat surface.
//!
//! HTTP providers execute these tools in-process; Claude CLI receives the same
//! vault through `continuum-mcp`. Both paths must obey the effective native
//! permission policy and the same sensitive-memory egress boundary.
...
/// In-process vault executor used by OpenAI-compatible and Anthropic adapters.
...
fn resolve_mcp_binary() -> Option<PathBuf> {
    ...
    WARN_ONCE.call_once(|| {
        tracing::warn!(
            layer = "desktop",
            component = "chat_tools",
            "continuum-mcp binary not found — Claude CLI chat runs without memory tools"
        );
    });
    None
}
...
mod tests {
    ...
    #[test]
    fn tool_definitions_cover_the_four_memory_tools() {
