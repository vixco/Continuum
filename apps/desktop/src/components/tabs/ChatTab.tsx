...
function ChatWorkspace() {
  ...
  const setVoicePartial = useChatStore((s) => s.setVoicePartial);
    ...
    <div className="flex h-full min-h-0">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-bg">
        ...
        <VoiceInputBubble
          ...
          onCommit={(t) => {
            endVoice(t);
            // Auto-send after voice commit: the orchestrator has the
            // transcript, so there's no value in forcing a manual click.
            if (t) setTimeout(() => void useChatStore.getState().send(), 50);
          }}
        />
        {/* Wire the partial-transcript updater; in this iteration the
            runtime writes a stub transcript from the VAD loop so the
            bubble shows life, but the actual STT still comes through
            the orchestrator when the user releases the mic. */}
        <VoiceTranscriptStub active={voiceListening} onPartial={setVoicePartial} />
      </div>
    </div>
  );
}
...
/**
 * VoiceTranscriptStub — receives a partial transcript and pushes it into
 * the store so the VoiceInputBubble can show life while the user is
 * speaking. In the current phase this is a no-op loop that produces a
 * gentle placeholder so the bubble is visibly animating; the real
 * partial-transcript stream ships when the WebSpeech fallback lands.
 */
function VoiceTranscriptStub({
  active,
  onPartial,
}: {
  active: boolean;
  ...
  useEffect(() => {
    ...
    let i = 0;
    const t = setInterval(() => {
      onPartial(hints.slice(0, (i % hints.length) + 1).join(""));
      i++;
    }, 700);
