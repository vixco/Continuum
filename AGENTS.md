...
cmd.arg("--model").arg(model_name); // "Codex-opus-4-6" or "Codex-sonnet-4-6"
...
- **You don't know the Codex CLI flags?** Check `code.Codex.com/docs/en/headless` or run `Codex --help`. Do not guess.
...
2. **Check if the problem is upstream.** Did a dependency version change? Did a crate API shift? Did Codex update its CLI?
