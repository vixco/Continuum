...
Workers are independent Claude Code sessions that Kairo's orchestrator spawns to do actual, possibly-long-running work. This document covers how workers are queued, how a model is chosen, how their lifecycle is observed, and how the audit trail makes every worker action reviewable after the fact.
...
- Given its own tool allowlist (narrower than the orchestrator's — no access to `mcp__kairo__workers_*`, so workers can't spawn more workers through MCP).
...
Orchestrator          (claude CLI + MCP tool call)
  └─► kairo-mcp       (workers_spawn_worker writes intent file)
        └─► kairo-core::workers::pool::WorkerPool
              ├─ WorkerSupervisor (spawns child claude process)
              └─ snapshot file  ──► dashboard + MCP workers_worker_status
```

The MCP server and the kairo runtime often live in different processes (the runtime owns the pool, the MCP server is re-spawned on every wake). They communicate through two disk folders:

- `~/.kairo-dev/worker-intents/` — JSON files the MCP server writes to request actions.
- `~/.kairo-dev/workers/` — per-worker JSON snapshots the runtime publishes.
...
All worker behaviour comes from `[workers]` in `~/.kairo-dev/config.toml` (defaults in `config/default-models.toml`):
...
| Tool                                   | Purpose |
|---------------------------------------|---------|
| `mcp__kairo__workers_spawn_worker`     | Queue a new worker; returns `worker_id` immediately |
| `mcp__kairo__workers_worker_status`    | Read one worker's current snapshot |
| `mcp__kairo__workers_worker_wait`      | Block until a worker reaches a terminal state |
| `mcp__kairo__workers_worker_cancel`    | Stop a running or queued worker |
| `mcp__kairo__workers_worker_list`      | List recent workers (filterable by status) |

All tools are documented in `docs/mcp-tools.md`.

## Skills and workers

When the pool launches a worker, it asks the active `SkillLoader` which skills apply (matching the task text + optional `skills: [...]` override from the spawn request). Matched skill content is concatenated with the base worker system prompt and written to `~/.kairo-dev/worker-prompts/<worker-id>.md`, which the supervisor passes to `claude --append-system-prompt-file`.
...
Tag-filter queries over episodic memory find a worker's full activity: `mcp__kairo__memory_query_episodic` with the query `"worker:<id>"`.
...
`KAIRO_WORKER_DRY_RUN=1` swaps the supervisor's claude CLI call for an in-process fake that produces a synthetic stream. Used by:

- `cargo test` (avoids live API hits)
- `cargo run --example worker_demo -p kairo-core` (local smoke test)
- The `workers` health probe (verifies the pool can actually spawn)

Set `KAIRO_WORKER_DRY_RUN=0` (or unset it) before running against the real claude CLI.
...
```json
{
  "tool": "mcp__kairo__workers_spawn_worker",
  "input": {
    "task": "Refactor crates/kairo-core/src/voice/streaming.rs to use the new SpeechController API. Keep public surface stable. Write tests.",
    "cwd": "F:/TRYORVIA/kairo-ai",
    "model": "power",
    "priority": "user",
