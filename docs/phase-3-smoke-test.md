...
- Speaking "Kairo, wat kan jij?" into the microphone
...
2. `--- KAIRO WAKING ---` appears
3. `KAIRO:` followed by streaming text (Opus's response)
...
- Verify by checking `~/.kairo-dev/episodic_db/` directory exists and has data

## Test 3: Personality check

Verify Opus responds as Kairo, not as "Claude" or a generic assistant:
...
- Kairo continues running (does NOT crash)
- Next wake cycle works normally

## Test 6: No API key / auth failure

Run without being logged in: `claude logout` then `cargo run --release --bin kairo`

**Expected:**
- Wake attempt logs an error
- Kairo continues in observe-only mode
- No crash, no panic
...
- [ ] Response reflects Kairo personality (not generic assistant)
- [ ] Cost per wake logged
- [ ] Wake stored in episodic memory
- [ ] Second wake retrieves first interaction
- [ ] Ctrl+C cleanly shuts down
- [ ] Subprocess crash doesn't crash Kairo
- [ ] All clippy/fmt pass: `cargo clippy -p kairo-core -- -D warnings`
