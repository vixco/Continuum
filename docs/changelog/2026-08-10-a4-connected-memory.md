...
- Added an explicit canonical observation-privacy admission boundary: `never_observe` is rejected before candidate/evidence construction, `local_only` is ineligible for automatic memory-candidate construction and requires a separate explicit-confirmation/manual path, and only eligible `cloud_allowed` evidence enters normal automatic consolidation.
...
- Added synthetic tests for durable promotion, transient rejection, privacy admission, sensitive-content handling, entity compatibility, contradiction/expiry, hybrid ranking and bounded traversal.
